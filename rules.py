...
class Action:
    ...
    def execute(self):
        ...
        if( 'override' not in chan['data']):
            chan['data']['override'] = False

        if chan['data']['override'] is False:
...
class RulesManager:
    def __init__(self, name, cloud, pulled_data):
        ...
        if(pulled_data is not None):
            ...
            matches = (entry for entry in pulled_data if (('channel_name' in entry) and (entry['channel_name'] == "rules")))
            if(matches[0] is not None):
                self.on_update(matches[0])
